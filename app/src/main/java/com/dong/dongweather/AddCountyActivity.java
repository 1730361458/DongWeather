...
public class AddCountyActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public static final String WEATHERNAME = "weather_name";
    /**
     * 城市等级
     */
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (currentLevel == LEVEL_PROVINCE){
                    ...
                    if ((selectedCountyList = LitePal.where("weatherId == ?",currentCounty.getWeatherId()).find(SelectedCounty.class)).size() == 0){
                        ...
                        if (WeatherMainActivity.INMODE == WeatherMainActivity.INMODE_DIRECT) {
                            //直接进入的
                            Intent sintent = new Intent();
                            sintent.putExtra(WEATHERID, currentCounty.getWeatherId());
                            sintent.putExtra(WEATHERNAME,currentCounty.getCountyName());
                            ...
                            editor.putString(WEATHERNAME,currentCounty.getCountyName());
                            editor.apply();
                        }
                        //发送广播通知listviewservice服务更新窗口
                        Intent intent = new Intent(AppWidgetManager.ACTION_APPWIDGET_UPDATE);
                        intent.putExtra(WEATHERNAME,currentCounty.getCountyName());
...
//                    startActivity(intent);
    ...
    public void onBackPressed() {
        ...
        } else if (WeatherMainActivity.INMODE == WeatherMainActivity.INMODE_INDIRECT) {
            SharedPreferences.Editor editor = getSharedPreferences("data",MODE_PRIVATE).edit();
